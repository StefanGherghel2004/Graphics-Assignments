...
void Lab2::Init()
{
    ...
    // Create a mesh box using custom data
    {
        vector<VertexFormat> vertices
        {
            ...
            VertexFormat(glm::vec3(0, 1,  0), glm::vec3(1, 0.5, 0)),
            VertexFormat(glm::vec3(1, 1,  0), glm::vec3(1, 0.5, 0)),
    ...
    // by definition!
    {
        ...
        CreateMesh("tetraedru simplu", vertices, indices);
    }

    {
        ...
        CreateMesh("tetraedru complex", vertices, indices);
    }
    ...
    // opposing vertex orientations.
    {
        ...
        // Actually create the mesh from the data
        CreateMesh("patrat dubios", vertices, indices);
    }
...

    {
        ...
        // centrul rosu
        vertices.push_back(VertexFormat(glm::vec3(0, 0, 0), glm::vec3(1, 0, 0)));
        ...
        for (int i = 0; i <= N; i++) {
            float angle = (float) i / (float)N * _2PI;
            float x = cx + r * cosf(angle);
            float y = cy + r * sinf(angle);
            vertices.push_back(VertexFormat(glm::vec3(x, y, 0), glm::vec3(1, 0, 0)));
        }
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(-5.0f, 0, 0));

    // TODO(student): Draw the tetrahedron

    RenderMesh(meshes["tetraedru simplu"], shaders["VertexColor"], glm::vec3(-10.0f, 0, 0));
    RenderMesh(meshes["tetraedru complex"], shaders["VertexColor"], glm::vec3(-15.0f, 0, 0));

    // TODO(student): Draw the square
    RenderMesh(meshes["patrat dubios"], shaders["VertexColor"], glm::vec3(-20.0f, 0, 0));


    RenderMesh(meshes["CERC"], shaders["VertexColor"], glm::vec3(5.0f, 0, 0));

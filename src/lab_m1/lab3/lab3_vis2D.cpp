...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_Z)) {
        logicSpace.x += logicSpace.width * zoom / 2;
        logicSpace.y += logicSpace.height * zoom / 2;
        logicSpace.width -= logicSpace.width * zoom;
        logicSpace.height -= logicSpace.height * zoom;
    }

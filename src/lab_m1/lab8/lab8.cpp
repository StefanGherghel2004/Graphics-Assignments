...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int light_spot = glGetUniformLocation(shader->program, "spotLight");
    glUniform1f(light_spot, spotLight);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        {
            lightDirection = glm::normalize(glm::rotate(glm::mat4(1.0f), rotSpeed * deltaTime, glm::vec3(0, 1, 0)) * glm::vec4(lightDirection, 0));
        }
        ...
        {
            lightDirection = glm::normalize(glm::rotate(glm::mat4(1.0f), rotSpeed * deltaTime, glm::vec3(1, 0, 0)) * glm::vec4(lightDirection, 0));
        }
        ...
        if (window->KeyHold(GLFW_KEY_1)) cutOff += angleSpeed * deltaTime;
        if (window->KeyHold(GLFW_KEY_2)) cutOff -= angleSpeed * deltaTime;

...
void Lab9::Update(float deltaTimeSeconds)
{
    ...
    glUniform1i(glGetUniformLocation(shaders["LabShader"]->program, "rotate"), 1);
    ...
    glUseProgram(shaders["LabShader"]->program);
    glUniform1i(glGetUniformLocation(shaders["LabShader"]->program, "rotate"), 0);
    ...
    glm::vec3 quadPos = glm::vec3(0.0f, 0.5f, 0.0f);
    glm::vec3 camPos = GetSceneCamera()->m_transform->GetLocalPosition();
    ...
    float angle = glm::atan2(dir.y, dir.x);
...

    {
        ...
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f));
        RenderSimpleMesh(meshes["square"], shaders["LabShader"], modelMatrix, mapTextures["crate"], mapTextures["earth"]);
    }
